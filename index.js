class Grid {
  constructor(rectangles) {
    this.GRID_SIZE = Math.ceil(Math.sqrt(rectangles.length));

    const maxXY = Math.max(...rectangles.map((v) => Math.max(...v)));

    this.CELL_SIZE = Math.ceil((maxXY + 1) / this.GRID_SIZE);
